...
\label{sec:Timbre-Parameterisation}
	...
	\label{sec:Timbre-Parameterisation-DimensionalityReduction}
		...
		dimensions (the Principal Components, \acrshort{pc}s). This transformation is applied such that the first
		\acrshort{pc} accounts for the largest amount of variance in the data as possible. Each successive
		\acrshort{pc} then describes as much of the remaining variance as possible. The \acrshort{pc}s of a $n$
		dimensional dataset comprising $m$ observations are calculate in six steps:
	...
	\label{sec:Timbre-DimensionalityReduction-DimensionCorrelations}
		...
		describing the temporal fluctuation of frequency components in a specific band). Regression is used to
		model how each of these features affect the perception of each timbral dimension. The `Activity' dimension
		is contributed to by the high-low energy ratio and the sub-band flux in the 3\super{rd}, 6\super{th} and
		...
		etc.) the stimuli were produced by.  Alternatively, clustering can be applied to the results taken from an
