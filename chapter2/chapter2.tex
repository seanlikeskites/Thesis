...
\label{sec:Timbre-PsychoacousticPrinciples}
	...
	\label{sec:Timbre-PsychoacousticPrinciples-Frequency}
		...
		Hz. In music theory the frequency difference between two sounds is described in terms of tones and octaves.
		This musical notation assumes that the ratio of between two frequencies defines the perceived interval
		...
		perception of frequency by human listeners. \citep{stevens1937a} propose a new frequency scale, the Mel
		scale, on which a given change in frequency produces the same perceived interval.
		...
		the spectral moments discussed in Section \ref{sec:Timbre-LowLevelFeatures-Spectral}. From their first use
		in speech recognition MFCCs have been added to the corpus of audio features used by researchers
		investigating timbre \citep{depoli1997sonological}. 
	...
	\label{sec:Timbre-PsychoacousticPrinciples-CriticalBands}
		...
		pass filters. Each of these auditory filters represents the region of the membrane excited by a sinusoidal
		tone at its centre frequency. The bandwidth of these filters is known as the critical bandwidth. Portions
		of signals will interfere with the perception of others which are within one critical bandwidth.
		...
		frequency of the tones. As the difference in frequencies gets greater than 15Hz this amplitude modulation
	...
	\label{sec:Timbre-PsychoacousticPrinciples-SpecificLoudness}
		...
		these bands is calculated. The procedure for doing this is beyond the scope of this work. A description
		...
		bands, each one critical bandwidth wide. These bands are known as the bark bands and provide a perceptual
