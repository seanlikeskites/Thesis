...
\label{sec:Timbre-LowLevelFeatures}
	...
	\label{sec:Timbre-LowLevelFeatures-Spectral}
		...
		of the $n$\super{th} spectral component are denoted $a_{n}$ and $\nu_{n}$ respectively. Spectral components
		...
		\subsubsection*{Spectral Moments}
			...
			The kurtosis measures the length of a distributions tails. A low kurtosis describes a spectrum where
		...
		\subsubsection*{Spectral Flatness}
			...
			Where $L_{n}$ is the $n$\super{th} element of list $L$. The two means are only equal if all elements
		...
		\subsubsection*{Tristimulus}
			...
			Where $h_{n}$ is the amplitude of the $n$\super{th} harmonic in the signal and $N$ the total number
			of harmonics.
		...
		\subsubsection*{Inharmonicity}
			...
			This produces values between zero and one. Zero describing a signal in which all partials are
			harmonics of the fundamental and one describing a signal where all partials have frequencies lying
