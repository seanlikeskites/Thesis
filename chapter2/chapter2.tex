...
\label{sec:Timbre-Introduction}
	...
	what timbre is and its contributing factors. There are three properties that describe the way a sound is perceived,
	these being loudness, pitch and timbre.  Loudness describes the perceived intensity of a sound and pitch its
	perceived frequency. Timbre then describes any other properties of a sound, besides loudness and pitch, which allow
	it to be distinguished from other sounds \citep{mathews1999introduction}. The relationship between perceived
	...
	This chapter will review the existing body of timbral research. Early research into timbre was performed by
...
\label{sec:Timbre-PsychoacousticPrinciples}
	...
	\label{sec:Timbre-PsychoacousticPrinciples-Frequency}
		...
		octaves.  This musical notation assumes that the ratio of two frequencies defines the perceived
		...
		the perception of frequency by human listeners. \citet{stevens1937a} propose a new frequency scale, the mel
...
\label{sec:Timbre-TimbralFeatures}
	...
	\label{sec:Timbre-TimbralFeatures-Roughness}
		...
		dissonance of tones. The history of this work is summarised by \citet{plomp1965tonal}. In review of this
		work and the results of listening experiments, they conclude that dissonance is manifested as a sensation
		of roughness due to amplitude fluctuations. They also suggest a relationship to critical bandwidth, stating
		that the most dissonant (roughest) interval between tones is one quarter of the critical bandwidth.
		\citet{vassilakis2010psychoacoustic} suggest that the sensation of roughness depends on more of the
...
\label{sec:Timbre-ListeningTests}
	...
	\label{sec:Timbre-ListeningTests-Methods}
		...
		\subsubsection*{Attribute Scale Tests}
			...
			the adjectives for the test. The adjectives used by \citet{kendall1993verbal1}, shown in Table
			\ref{tab:vonBismarcksDescriptors} were taken from earlier work performed by
			...
			experiment than when the adjectives were selected by the experimenter, as in the first experiment.
	...
	\label{sec:Timbre-ListeningTests-Environments}
		...
		\subsubsection*{Distributed Listening Tests}
			...
			section to ensure a participants reproduction system is suitable for the tests. This extra section
			assesses the range of frequencies a participant's system can reproduce and excludes those which do
...
\label{sec:Timbre-Parameterisation}
	...
	\label{sec:Timbre-Parameterisation-DimensionalityReduction}
		...
		VAME experiment where $N$ adjective scales are used consist of an $N$ dimensional space in which each
...
\label{sec:Timbre-Control}
	...
	\label{sec:Timbre-Control-ParameterSpaces}
		...
		avoided by analysing the audio features of the signals as discussed in Section
		\ref{sec:Timbre-Control-TimbreSpaces}. Using this method, the necessary audio feature manipulation is
		determined and then the parameters of the audio effect are set accordingly.
...
\label{sec:Timbre-AudioProcessing}
	...
	\label{sec:Timbre-AudioProcessing-Classification}
		...
		or timbral characteristics. \citet{verfaille2006an} suggest that each of these methods are each appropriate
		when discussing different tasks involving audio effects. When considering the development of audio effects
		it is meaningful to group effects based on the signal processing techniques used in their construction. In
		contrast, end users of the effects (music producers, musicians etc.) may find it more convenient to
		classify effect by the perceptual characteristics they impart on a signal. \citet{eichas2015feature}
		classify audio effects using the mathematical properties of the underlying system: whether the system is
		linear and whether or not the output is dependent on time. This classification provides a compromise
		between the previous two methods, grouping effects into classes which are similarly implemented and also
		have similar timbral effects.
		...
		effects are gathered from the dataset collected by the SAFE project (discussed in detail in Section
		\ref{sec:TimbreEvaluation-DAWBasedTimbreEvaluation-SAFE}).
	...
	\label{sec:Timbre-AudioProcessing-LTI}
		...
		of audio effects used by audio engineers equalisation, delay and reverb are based on LTI systems.
