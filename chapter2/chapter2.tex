...
\label{sec:Timbre-Parameterisation}
	...
	\label{sec:Timbre-AuditoryEvaluation}
		...
		they are given to rate dissimilarity on during the experiment. Triadic comparisons just asks which pair is
		most similar and which the least avoiding the elicitation of a value on a scale.
	...
	\label{sec:Timbre-DimensionalityReduction}
		...
		(such as the euclidean distances between them).

		\note
		{
			How much depth to to into here? 

			\citet{grey1977multidimensional} uses MDS as people tend to on dissimilarity data.
			\citet{kendall1993verbal1} use MDS on their VAME data.

			But everybody seems to use a slightly different version of MDS (clascal, indscal, isomap).
			\citet{mcadams1995perceptual} covers some differences.

			\cite{zacharakis2012analysis} prefer EFA over PCA but also use some clustering.

			\citet{burgoyne2008a} performed a meta analysis of several other researchers results.
		}


	\subsection{Correlation of Dimensions with Audio Features}
	\label{sec:Timbre-DimentionCorrelations}
		\note
		{
			Several researchers calculate correlation between audio features and timbre space dimensions.

			\citet{mcadams1995perceptual} find correlations with log attack time and spectral centroid.

			\citet{caclin2005acoustic} find correlations with the features which they changed (log attack,
			spectral centroid and spectral flux).
	
			\citet{marui2005timbre} find correlations between timbre space dimensions and scores on the semantic
			differential scales.

			\citet{alluri2010exploring} perform regression to see the contributions of audio features to
