...
\label{sec:Timbre-LowLevelFeatures}
	...
	\label{sec:Timbre-LowLevelFeatures-Temporal}
		...
		\subsubsection*{Amplitude Envelope Analysis}
			...
			effect of the perceived timbre of a sound. \citet{patterson1994the} show that signals with similar
			spectral content but different amplitude envelopes had be differentiated.
...
\label{sec:Timbre-AudioProcessing}
	...
	\label{sec:Timbre-AudioProcessing-LTI}
		...
		\subsubsection*{Equalisation}
			...
			equalisers with more control parameters have been introduced \citep{gottlieb2007shaping}. These
			modern equalisers are still used for corrective purposes but also provide more opportunities for
			the creative shaping of timbre. Responses from the SAFE project (Section
		...
		\subsubsection*{Reverberation}
			...
			to enhance a musical production. The timbral characteristics of the reverb effect use in the SAFE
			project has been described with the terms roomy, damp, dreamy, airy, big, massive and distant.
	...
	\label{sec:Timbre-AudioProcessing-LTV}
		...
		a low frequency oscillator (LFO). \note{Flange, phase, chorus a la \citep{huber2010modern}}.
	...
	\label{sec:Timbre-AudioProcessing-Nonlinear}
		...
		processing parameters based on the amplitude of the input signal. Two common uses for nonlinear processing
		is control of dynamics and application of deliberate distortion.

		\subsubsection*{Dynamics Processing}
			\note{Gates, compressors, expanders}

			\note{punchy, tight, smooth, squashed, snappy and crushed.}
		...
		\subsubsection*{Nonlinear Distortion}
			...
			\note{crunchy, warm, fuzzy, creamy, bright, harsh, raspy and smooth.}
