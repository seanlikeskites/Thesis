...
\label{sec:Timbre-Introduction}
	...
	Some stimuli contain several perceived pitches (such as a chord played on a piano), while others do not have a
	pitch (such as white noise). Once the pitches present in a stimulus have been determined, they can be compared
	...
	pieces of music, they can not be applied generally to a wide range of timbres. It is not very useful to describe
	the timbre of a xylophone as being `not flute-like'. More general timbral descriptors directly describe the sound
	itself, rather than the source which produced it. These include terms such as bright, rough and sharp. The timbre
	of different sounds can then be compared using these terms \citep{howard2009acoustics}. Sounds can also be ordered
	in terms of these criteria, much like with loudness and pitch. For example, one could arrange a set of sounds in
	order of their perceived brightness.

	Early research into timbre was performed by \citet{helmholtz1875on}, who investigated the perceived nature of
...
\label{sec:Timbre-LowLevelFeatures}
	...
	\label{sec:Timbre-LowLevelFeatures-Temporal}
		...
		mean value of a sinusoidal signal, over a whole number of cycles, is zero. A common technique to extract
		more meaningful information about a signal's level is to use an envelope detector. An envelope detector
		produces an envelope for a signal: a smooth line which describes the peaks or troughs in a signal, ignoring
		the oscillatory changes in amplitude due to its frequency. Applying an envelope detector to the temporal
		...
		\subsubsection*{Envelope Detection}
			...
			rectification is applied to produce a unipolar signal; then a low pass filter is applied to smooth
			the result \citep{dutilleux2011modulators}. In the digital domain more complex envelope detectors
			are possible, such as the instantaneous amplitude detector. Instantaneous amplitude is calculated
			as the absolute value of an analytic representation of the input signal. An analytic signal is a
			complex valued signal, the real part of which is the original signal and the imaginary part its
			Hilbert transform. The analytic signal is often denoted with a subscript letter $a$, such that the
			analytic representation of the signal $x[n]$ would be denoted $x_{a}[n]$. This method, along with
			more complex digital envelope detectors, is reviewed by \citet{chang2007a}. 
			...
			give a more accurate filter but introduce more delay and increase the overall complexity.  More
			efficient implementations can be built using \acrshort{iir} filters if some of the properties of an
			ideal Hilbert transform are disregarded.  \citet{oppenheim2014discrete} suggest constructing a
			phase splitter: processing audio with two parallel allpass \acrshort{iir} filters, the phase
		...
		\subsubsection*{Amplitude Envelope Analysis}
			...
			effect on the perceived timbre of a sound. \citet{patterson1994the} showed that signals with
			similar spectral content, but different amplitude envelopes, can be differentiated by listeners.
		...
		\subsubsection*{Other Temporal Audio Features}
			...
			\citet{wilson2014profiling} in which the \acrfull{pmf} of sample amplitudes in a segment of audio
			is used as a measure of the distortion present in the signal.  Temporal features on this scale are
			outside of the scope of this work as they no longer relate to the timbre of individual sounds but
			rather the perceived nature of a musical piece.
	...
	\label{sec:Timbre-LowLevelFeatures-Spectral}
		...
		of the $n$\super{th} harmonic being denoted $h_{n}$. No additional notation is required for the frequency
		...
		\subsubsection*{Spectral Moments}
			...
			spectrum in which small amounts of energy lie at frequencies far away from the central band.
		...
		\subsubsection*{Spectral Irregularity}
			...
			This is often implemented using the raw amplitudes of the partials, rather than their dB
			amplitudes, yielding Equation~\ref{eq:LibXtractKrimphoffIrregularity}. This is how the Krimphoff
			irregularity is implemented in LibXtract \citep{bullock2007libxtract} (a software library for audio
			analysis used throughout this work).
		...
		\subsubsection*{Tristimulus}
			...
			be used on more inharmonically structured signals. The spectrum is still split into three bands,
			but the number of partials within these bands depends on the content of the signal. These bands
			include all partials with a frequency within $0.5f_{0}$Hz of the harmonics in the equivalent band
			used in the calculation of tristimulus.  The peak tristimulus measures are given by Equations
		...
		\subsubsection*{Odd to Even Harmonic Ratio}
			...
			harmonics, where a division by zero will occur. A more robust method is to use separate measures
			for the proportion of odd harmonics in a signal and the proportion of even harmonics in a signal.
...
\label{sec:Timbre-PsychoacousticPrinciples}
	...
	\label{sec:Timbre-PsychoacousticPrinciples-Frequency}
		...
		perception of frequency by human listeners. \citet{stevens1937a} proposed a new frequency scale, the mel
		...
		bands. An example of this are the \acrfullpl{mfcc}, developed for use in speech recognition
		...
		\acrfull{dct} of the log powers of these bands is then calculated to produce the \acrshortpl{mfcc}. From
		their first use in speech recognition, \acrshortpl{mfcc} have been added to the corpus of audio features
...
\label{sec:Timbre-TimbralFeatures}
	...
	\label{sec:Timbre-TimbralFeatures-Roughness}
		...
		The psychoacoustic models presented in this section are restricted to modelling descriptors for which
...
\label{sec:Timbre-ListeningTests}
	...
	\label{sec:Timbre-ListeningTests-Methods}
		...
		\subsubsection*{Dissimilarity Tests}
			...
			\citet{caclin2005acoustic} also used a pairwise dissimilarity method, but, unlike
		...
		\subsubsection*{Attribute Scale Tests}
			...
			perceptual differences between stimuli, but also for descriptors to be applied to characterise
			these differences, as done by \citet{zacharakis2014an}.
	...
	\label{sec:Timbre-ListeningTests-Environments}
		...
		run supervised listening tests using specific reproduction equipment in a laboratory; or to produce a web
		...
		\subsubsection*{Laboratory Listening Tests}
			...
			same environment, at the same level. This removes any variability in these areas, such that any
		...
		\subsubsection*{Distributed Listening Tests}
			...
			to the researcher. This is most easily implemented over the internet, via a web interface which
			runs the test and automatically returns the results to a server. Distribution of the testing
			interface facilitates easier collection of data from a large sample of people as participants are
			able to take the test at a time which suits them. For laboratory tests, the participants,
			experimenter and testing equipment all need to be available at the same time in order to carry out
			a test.
...
%			experimenter to work on other tasks.
			...
			Once data has been gathered from a perceptual listening test, one must analyse it to determine the
...
\label{sec:Timbre-Parameterisation}
	...
	\label{sec:Timbre-Parameterisation-DimensionalityReduction}
		...
		\acrshort{mds} is typically used in situations where dissimilarity data is being analysed. This type of
		...
		dimensions (the \acrfullpl{pc}). This transformation is applied such that the first \acrshort{pc} accounts
		...
		\begin{enumerate}
			...
			\item Standardise the data, creating a new $m \times n$ matrix, $\mathbf{B}$, in which each column
				has a mean of 0 and a standard deviation of 1. The $i$\super{th} column of $\mathbf{B}$ is
				calculated from the $i$\super{th} column of $\mathbf{A}$ using
			...
			\item \label{enum:Eigen} Find the eigenvalues of the matrix $\mathbf{C}$ and arrange them in
				...
				$i$\super{th} column of which is the eigenvector associated with the eigenvalue
				$\lambda_{i}$.
		...
		The rows of $\mathbf{E}$ represent the coordinates of each observation in each of the \acrshort{pc}s. Since
		...
		where $\lambda_{i}$ is the $i$\super{th} largest eigenvalue of the covariance matrix, $\mathbf{C}$, as
	...
	\label{sec:Timbre-DimensionalityReduction-DimensionCorrelations}
		...
		\citet{alluri2010exploring} took a more in depth approach, using stepwise and principal component
		...
		ratio and the sub-band flux in the 3\super{rd}, 6\super{th} and 8\super{th} of 10 spectral bands. The
		`Brightness' dimension comprised zero crossing rate and the 1\super{st} and 3\super{rd} bands' fluxes; and
		the `Fullness' dimension was described by zero crossing rate, high-low energy ratio, entropy and the
		2\super{nd} band's flux.
...
\label{sec:Timbre-Control}
	...
	\label{sec:Timbre-Control-TimbreSpaces}
		...
		created to synthesise sounds at particular positions within a timbre space, or to manipulate existing
		sounds to alter their location in the timbre space.
		...
		distribution of the spectrum, and articulation and envelope parameters which control the temporal
		properties of the signal's amplitude envelope. They state that this system assisted novice users but proved
		too limited for experienced users, who desired a finer degree of control.
	...
	\label{sec:Timbre-Control-ParameterSpaces}
		...
		asking listening test participants to rank how well given semantic descriptors characterise an audio
		stimulus processed with several different parameter configurations. Weightings for each effect parameter
		could then be deduced by looking at the correlations of parameter values and their respective rankings.
...
\label{sec:Timbre-AudioProcessing}
	...
	\label{sec:Timbre-AudioProcessing-LTI}
		...
		\subsubsection*{Reverberation}
			Reverberation (reverb) simulates the effects of the natural reflection of sounds in enclosed
			...
			of reverb are often described using size / location descriptors (`roomy', `big', `massive',
			`distant'), or more abstract terms (`damp', `dreamy', `airy').
	...
	\label{sec:Timbre-AudioProcessing-Nonlinear}
		...
		\subsubsection*{Dynamics Processing}
			...
			describe the application of compression in the \acrshort{safe} dataset are: `punchy', `tight',
			`smooth', `squashed', `snappy' and `crushed'.
		...
		\subsubsection*{Nonlinear Distortion}
			...
			\acrshort{safe} project have been described as `crunchy', `warm', `fuzzy', `creamy', `bright',
			`harsh', `raspy' and `smooth'. As the focus of this thesis is the application of nonlinear
			distortion for timbral control, an in depth discussion of nonlinear distortion is given in
			Chapter~\ref{chap:Excitation}. 
