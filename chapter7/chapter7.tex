\chapter{Perceptual Experiments} % probably change the title.
...
\label{sec:PerceptualExperiments-Introduction}
	...
	This chapter presents two experiments that evaluate the performance of harmonic excitations systems based on those
	...
	In both experiments the performance of the systems is evaluated using both objective metrics and subjectively
	through perceptual listening tests.
...
\label{sec:PerceptualExperiments-SemanticControl}
	...
	\label{sec:PerceptualExperiments-SemanticControl-EffectDesign}
		...
		groups these descriptors belong to. The first effect, the warmth / brightness effect, is intended to alter
		...
		`harshness'. The second effect, the brightness / crunchiness effect, is intended to alter timbre between
		the between `brightness' and `crunchiness' groups, introducing `crunchiness' for high parameter values and
		...
		\subsubsection*{Warmth / Brightness}
			...
			$f_{0}$ of the input signal and high pass filtering at the input's spectral centroid.  Separating
			the bands at the spectral centroid in this way ensures that their respective spectral centroids sit
			either side of the input's. Full wave rectification is used to ensure the system is positive
			homogeneous.
		...
		\subsubsection*{Brightness / Crunchiness}
			...
			spectral skewness. To attempt to recreate this the brightness / crunchiness effect controls the
	...
	\label{sec:PerceptualExperiments-SemanticControl-Discussion}
		\subsubsection*{Warmth / Brightness Effect}
			...
			effect of the warmth / brightness effect when its parameter is set to `warm', followed by `clear',
			...
			shows that, when describing the effect of the warmth / brightness effect, participants used a
		...
		\subsubsection*{Brightness / Crunchiness Effect}
			...
			which cluster closer to `crunch' in the feature difference space when labelling the brightness /
	...
	\label{sec:PerceptualExperiments-SemanticControl-Conclusion}
		...
		also used to describe these effects suck as `fuzzy' and `raspy'. These terms were associated with the SAFE
