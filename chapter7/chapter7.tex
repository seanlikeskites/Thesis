...
%
		...
		Comparing these observations with the results of the multiple stimulus listening test, it appears that the
		relationships between the amplitudes of the harmonics in the original signal are not the most important
		factor in the perceived quality of the reconstruction. Aside from those reconstructed from the piano
		signal, the maximum mean grade for any of the reconstructed signals was approximately 60. It might be
		expected that because the harmonics in the sustain section of the cello signal have the most similar
		amplitude envelopes to the fundamental it would receive higher average score due to fewer inaccuracies
		being introduced by the reconstruction. It might also be expected that removing the amplitude modulation of
		the harmonics in the synthesised signal would decrease the perceived quality of the reconstruction. These
		expectations are not met in the results, suggesting that another factor is more prominent in the accuracy
