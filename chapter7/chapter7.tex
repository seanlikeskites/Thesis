...
\label{sec:PerceptualExperiments-Reconstruction}
	...
	\label{sec:PerceptualExperiments-Reconstruction-Methodology}
		...
		\subsubsection*{Objective Evaluation}
			...
			measured using the $R_{\textrm{nonlin}}$ metric proposed by \citet{tan2004predicting}. As discussed
	...
	\label{sec:PerceptualExperiments-Reconstruction-Results}
		...
		The $R_{\textrm{nonlin}}$ results for each of the stimuli are presented in Figure \ref{fig:SMCRNonlin}. The
		...
		$R_{\textrm{nonlin}}$ score lower than that of the anchor. This is due to the piano signal having a damped
		fundamental.  Reconstructing the higher order harmonics using information in the fundamental produces lower
		quality harmonics as the structure of the fundamental does not reflect that of the other harmonics in the
		signal. In lieu of this, test participants who grade any of the reconstructed piano stimuli less than the
		...
		intervals. The Pearson correlation ($r$) between these mean grades and the $R_{\textrm{nonlin}}$ scores for
		...
		\begin{figure}[h!]
			...
			\caption{$R_{\textrm{nonlin}}$ values for each of the stimuli.}
		...
		\begin{table}[h!]
			...
			\caption{Correlations between the $R_{\textrm{nonlin}}$ values and the mean grades.}
...
%
		...
		reconstruction as the STFT window length / filter order is increased. The $R_{\textrm{nonlin}}$
		...
		the $R_{\textrm{nonlin}}$ measures it is seen that the IAP reconstructions of this signal are objectively
...
\label{sec:PerceptualExperiments-SemanticControl}
	...
	\label{sec:PerceptualExperiments-SemanticControl-Conclusion}
		...
		described as `bright' and `harsh' by isolating the fundamental, applying full wave rectification and high
...
\label{sec:PerceptualExperiments-Conclusion}
	...
	using the amplitude envelope of the fundamental frequency in the reconstruction of harmonics. It has been shown
	that the degree to which the amplitude envelopes of the harmonics in a signal differ from that of the fundamental
	dictates the perceived quality of a reconstruction using these systems. If additional prior information is known
	about the relationships between the harmonics in the signal being reconstructed the quality of the reconstruction
	could be improved by selecting different harmonics to base the reconstructed amplitude envelopes on.
