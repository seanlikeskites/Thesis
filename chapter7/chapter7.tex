...
\label{sec:PerceptualExperiments-SemanticControl}
	...
	\label{sec:PerceptualExperiments-SemanticControl-Results}
		\subsubsection{Mahalanobis Distances}
			...
			successful. This is possibly explained by the simplifications made to reduce the effects
			computational complexity. Using lower order filters increases the level of intermodulation
		...
		\subsubsection{Listening Test Results}
			...
			The results of the listening tests can also be analysed by comparing them to the clustering
			performed in Section \ref{sec:TimbreEvaluation-Analysis-TermClustering}. The proximity of the users
			response to the desired response can be measured as the proximity of the descriptor they responded
			with to the desired descriptor in the dendrograms shown in Figure \ref{fig:CombinedClusters}. This
			is measured as the cophenetic distance between the clusters in which the two descriptors lie. Where
			a descriptor appears twice in the dendrogram (from both the distortion and equaliser) the
			combination of points which yield the lowest cophenetic distance is used. Figures
			\ref{fig:HarshCophs} and \ref{fig:CrunchCophs} show the mean cophenetic distances for each
			instrument, effect and parameter setting combination in both the processed feature and feature
			difference clustering dendrograms (Figures \ref{fig:CombinedProcessedClusters} and
			\ref{fig:CombinedDifferenceClusters}).
