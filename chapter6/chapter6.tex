...
\label{sec:FeatureControl-Parameterisation}
	...
	\label{sec:FeatureControl-Parameterisation-Inharmonicity}
		...
		\subsubsection*{Manipulation of Inharmonicity}
			The system given in Figure \ref{fig:InharmonicitySystem} can be used to generate two signals with
			different inharmonicities. For the purpose of demonstration the first nine harmonics of the tests
			signals were filtered out and reconstructed in two different manners. The first reconstruction
			applied no frequency shift to the excited harmonics, generating a signal with low inharmonicity.
			The second reconstruction generated harmonics with linearly increasing deviation from harmonic
			frequencies ($0.1f_{0}$ for the second harmonic up to $0.5f_{0}$ for the ninth harmonic). These two
			signals were then summed with gains defined by a parameter $P$, the harmonic signal being
			multiplied by a gain of $1 - P$ and the inharmonic signal by $P$. Figure
			\ref{fig:MoveInharmonicities} show the result of altering this parameter on the inharmonicity of
			...
			\note{Synth contains a lot of perfectly harmonic high order harmonics.}
