...
\label{sec:FeatureControl-Parameterisation}
	...
	\label{sec:FeatureControl-Parameterisation-HarmonicParityRatio}
		...
		\begin{equation}
			m = \sqrt{\frac{\textrm{OER}\sum_{1 \leq n \leq N, n \textrm{ is even}} h_{n}^{2}}
				       {\sum_{1 \leq n \leq N, n \textrm{ is odd}} h_{n}^{2}}},
				       \quad \textrm{OER} \geq 0;
		...
		$\textrm{OER}$ of the clarinet signal seems to be the easiest to control. This can be attributed to the
		...
		The cello and synthesised signals both exhibit a limit on how high the $\textrm{OER}$ can be made. This
		...
		decrease in $\textrm{OER}$ seen for the synthesised signal is due to destructive superposition between
		the harmonics in the three signals being summed.

		The system discussed here provides no control over the $\textrm{OER}$ of the piano signal. This is again
		...
		harmonics, meaning the resulting signal will have a high evenness, or low $\textrm{OER}$.
