...
\section{Summary}
\label{sec:FeatureControl-Summary}
	\note
	{
		Introduce the table any why it is a good thing and what the columns refer to. All things require f0
		tracking but other elements may differ.
	}
	...
	\begin{table}[h!]
		...
		\begin{tabular}{|C{3cm}|c|C{5cm}|C{3cm}|}
			\hline
			\bf{Feature} & \bf{System} & \bf{Nonlinear Device} & \bf{Monotonicity} \tabularnewline
			\hline
			\hline
			Spectral Centroid, Skewness and Kurtosis &
			Figure~\ref{fig:TwoBandSpectralCentroidSystem} & 
			Any which generate a large band of harmonic energy, e.g. static nonlinearities. & 
			Monotonic Control \tabularnewline
			\hline
			...
			Individual harmonic generation techniques (e.g. SSBA or IAP). & 
			Monotonic control for a particular parameter range. \tabularnewline
			\hline
			Spectral Slope & Figure~\ref{fig:SuperpositionSystem} &
			Individual harmonic generation techniques (e.g. SSBA or IAP). & 
			Linear Control \tabularnewline
			\hline
			Tristimulus & Figure~\ref{fig:SuperpositionSystem} & 
			Individual harmonic generation techniques (e.g. SSBA or IAP). & 
			Monotonic Control \tabularnewline			
			\hline
			$\mathrm{OER}$ & Figure~\ref{fig:HarmonicParitySystem} &
			Nonlinearities which can generate bands of odd or even harmonic (e.g. static nonlinearities
			described by odd or even characteristic curves). & 
			Monotonic Control \tabularnewline
			\hline
		...
		\label{tab:FeautreControlSummary}
	\end{table}
