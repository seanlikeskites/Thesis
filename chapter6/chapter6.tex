...
\label{sec:FeatureControl-Introduction}
	...
	linear control requires in depth analysis of the input signal, which cannot be applied in real time. Additionally,
...
\label{sec:FeatureControl-Systems}
	In Section~\ref{sec:Excitation-Methods}, several methods of introducing new frequency content to a signal were
	introduced. In Section~\ref{sec:ExcitationEvaluation-Comparison}, these methods were discussed further and the
	...
	\label{sec:FeatureControl-Systems-Fundamental}
		...
		For this system to operate in real time, the $f_{0}$ of the signal must be calculated in real time.
	...
	\label{sec:FeatureControl-Systems-Individuals}
		...
		\begin{figure}[h!]
			...
			\begin{tikzpicture}
				...
				\node (Exciter) [draw] at (4.25, 1) {Shifter};
				\draw (Filter) -- (Exciter);
				\draw (Side) -- (F0);

				\node (Out) at (5.75, 1) {$y[n]$};
				\draw (Exciter) -- (Out);
			\end{tikzpicture}
		...
		A complex spectrum can be created by generating several harmonics of different orders in this manner, and
	...
	\label{sec:FeatureControl-Systems-SpectralShaping}
		...
		in Figure~\ref{fig:HarmonicGenerationSystem}. This allows the majority of the harmonics to be excited
		efficiently, only using more complex processing where it is needed. \citet{howard2009acoustics} showed that
		low order harmonics sit in separate critical bands, whereas high order harmonics are within one critical
		bandwidth of one another. This means that lower order harmonics are resolved separately by the human
		hearing system, while the high order harmonics are perceived as fused sounds. As the order of harmonics
		increases, their individual amplitudes have less effect on the perceived timbre of the sound. For this
		reason, a harmonic excitation system need only provide control over the individual amplitudes of low order
		harmonics. 
...
%		\end{gather}
	...
	\label{sec:FeatureControl-Systems-Superposition}
		...
		excited. For the system in Figure~\ref{fig:SpectralShapingSystem}, this can be implemented using a series
		of notch filters tuned to the frequencies of the first nine harmonics, and a low pass filter to attenuate
		any frequency above the ninth harmonic. These filters can be applied to the unexcited signal depending on
		which harmonics are being generated by the exciters. A diagram of a system which includes these filters is
		shown in Figure~\ref{fig:SuperpositionSystem}.
	...
	\label{sec:FeatureControl-Systems-Inharmonicity}
		...
		processing different classes of input signal. Such a system is shown in
...
\label{sec:FeatureControl-Parameterisation}
	...
	\subsection{Parameterisation Methodology}
	\label{sec:FeatureControl-Parameterisation-Methodology}
		...
		requires different processing techniques. For example, control of the \acrshortpl{mfcc} involves the
		...
		For each of these features, spectral manipulations are identified which will allow the value of the feature
		to be altered predictably. This manipulation is designed to have a single parameter, the features of the
		output signal being dependent on the value of this parameter and the features of the input signal.
		...
		properties of the input signal. Both the cello and clarinet signals represent typical recorded harmonic
	...
	\subsection{Feature Parameterisation Summary}
	...
	\label{sec:FeatureControl-Parameterisation-SpectralMoments}
		\subsubsection*{Spectral Centroid}
			...
			isolated $f_{0}$, followed by high pass filtering. These bands are filtered with corresponding low
		...
		\subsubsection*{Higher Order Spectral Moments}
			...
			Only controlling the amplitudes of the first nine harmonics (see
	...
	\label{sec:FeatureControl-Parameterisation-Inharmonicity}
		...
		Figure~\ref{fig:InharmonicitySystem} can be used to apply multiple approaches to this problem. 
		...
		the input only contains perfectly harmonic partials, or when the desired result is to reduce the
...
\label{sec:FeatureControl-Conclusions}
	...
	The parameterisations discussed all succeed in providing monotonic control over the audio feature in question. In
