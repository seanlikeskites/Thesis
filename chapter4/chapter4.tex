...
\label{sec:FeatureControl-Control}
	...
	\label{sec:FeatureControl-Control-SpectralMoments}
		As mentioned in Section \ref{FeatureControl-Parameterisation-SpectralMoments} introducing individual
		sinusoid is not an effective way to manipulate the spectral centroid. A band based approach is more
		favorable allowing the centroid to be moved upwards or downwards.
		\note
		{
			Single harmonics are not very good for this because they will dominate the spectrum. Generate two
			bands and use the summing technique from the Section
			\ref{sec:FeatureControl-Parameterisation-SpectralMoments}.
		}
	...
	\label{sec:FeatureControl-Control-Tristimulus}
		\note
		{
			Single harmonic generation reigns supreme. This is better than filters as it allows the features of
			the three bands to be manipulated as well.
		}
