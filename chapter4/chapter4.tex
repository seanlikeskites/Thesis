...
\label{sec:FeatureControl-Parameterisation}
	...
	\label{sec:FeatureControl-Parameterisation-SpectralCentroid}
		...
		\begin{equation}
			\frac{\sum_{n = 0}^{n-1} |Y[n]|}{\sum_{n = 0}^{n-1} |X[n]|} = 
	...
	\label{sec:FeatureControl-Parameterisation-Tristimulus}
		...
		from 0 to 1. 0 meaning there is no energy in that set of harmonics, 1 meaning the signal is entirely
		composed of those harmonics.
	...
	\label{sec:FetureControl-Parameterisation-HarmonicParityRatio}
		...
		\begin{equation}
			R_{OE} = \frac{\sum_{1 \leq n \leq N, n \textrm{ is odd}} h_{n}^{2}}
			              {\sum_{1 \leq n \leq N, n \textrm{ is even}} h_{n}^{2}}
			\label{eq:HarmonicParityRatio}
		\end{equation}

		\begin{equation}
			m = \sqrt{\frac{R_{OE}\sum_{1 \leq n \leq N, n \textrm{ is even}} h_{n}^{2}}
		...
		\note
		{
			This is really a crappy metric with a range of 0 to infinity.

			\citet{lukasik2005towards} maybe does something better.
		}
