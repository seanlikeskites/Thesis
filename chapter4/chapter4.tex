...
\label{sec:TimbreEvaluation-DAWBasedTimbreEvaluation}
	...
	\label{sec:TimbreEvaluation-DAWBasedTimbreEvaluation-SAFEOntology}
		...
		The Provenance Ontology is also used to describe the implementations of algorithms which produce audio
		feature data. Each \emph{safe:FeatureExtractionTransform} node is a \emph{prov:Activity} which can be
		associated with a particular implementation of the algorithm it uses (e.g. the LibXtract
		\citep{bullock2007libxtract} spectral centroid function). Say a discrepancy was found between two
		implementations of a given feature extraction algorithm; the provenance data allows queries to filter
		results, removing those generated using an earlier or incompatible implementation. Further use of
		provenance describes the role of a user in the application of a transform. Storage of this data allows the
		simple retrieval of all transforms associated with a particular user: benefiting researchers by allowing
		for segmentation of the data and benefiting users by allowing them to maintain personal libraries of
		semantic presets amongst the entire \acrshort{safe} dataset.
...
\label{sec:TimbreEvaluation-Analysis}
	...
	\label{sec:TimbreEvaluation-Analysis-TermClustering}
		Each of the terms arrived at, after stemming, in Section~\ref{sec:TimbreEvaluation-Analysis-Preprocessing}
		...
		\subsubsection*{Clustering of Distortion Terms}
			Figure~\ref{fig:DistortionClusters} shows dendrograms illustrating the clustering of terms in the
...
%			This figure is da baddest figure, ask your dyslexic down syndromed mother.
	...
	\label{sec:TimbreEvaluation-Analysis-TimbreSpaces}
		...
		\subsubsection*{Distortion Processed Features}
			...
			shown in Figures~\ref{fig:DistortionProcessedPCAs} and \ref{fig:DistortionProcessedCentroidsPCAs}.
			The corresponding agreement scores are shown in Table~\ref{tab:DistortionProcessedAgreements}. The
			three terms with the highest agreement scores (`warm', `fuzz' and `crunch') are of particular
			interest.
		...
		\subsubsection*{Distortion Feature Differences}
			...
			Figures~\ref{fig:DistortionDifferencePCAs} and \ref{fig:DistortionDifferenceCentroidsPCAs}. The
			corresponding agreement scores are shown in Table~\ref{tab:DistortionDifferenceAgreements}. This
			space is largely similar to that describing the distortion's processed signals
			(Figure~\ref{fig:DistortionProcessedPCAs}) and similar conclusions can be drawn.
		...
		\subsubsection*{Equaliser Processed Features}
			...
			Figures \ref{fig:EqualiserBWProcessedPCAs}, \ref{fig:EqualiserProcessedPCAs} and
			\ref{fig:EqualiserProcessedCentroidsPCAs}. The corresponding agreement scores are shown in
		...
		\subsubsection*{Equaliser Feature Differences}
			...
			\ref{fig:EqualiserBWDifferencePCAs}, \ref{fig:EqualiserDifferencePCAs} and
			\ref{fig:EqualiserDifferenceCentroidsPCAs}. The corresponding agreement scores are shown in
