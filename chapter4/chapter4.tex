...
\label{sec:FeatureControl-Parameterisation}
	...
	\label{sec:FeatureControl-Parameterisation-SpectralCentroid}
		...
		\begin{equation}
			\frac{\sum_{n = c + 1}^{N-1} \abs{X[n]}}{\sum_{n = 0}^{c} \abs{X[n]}} = 
			\frac{\mu_{l} - \mu}{\mu - \mu_{u}}, 
			\quad \mu_{l} \leq \mu < \mu_{u} \quad \textrm{or} \quad \mu_{u} < \mu \leq \mu_{l}
			\label{eq:SpectralCentroidManipulation}
		\end{equation}

		Where $\mu_{l}$ and $\mu_{r}$ are the spectral centroid of the upper and lower band, and $c$ is the cutoff
		bin between the two bands in the DFT.
	...
	\label{sec:FetureControl-Parameterisation-Inharmonicity}
		...
		respectively and $N$ the total number of partials of which the signal is composed.
	...
	\label{sec:FetureControl-Parameterisation-Flatness}
		...
		The arithmetic and geometric means of any set of non-negative numbers, $S$, satisfy the inequality in
		Equation \ref{eq:MeanInequality}.

		\begin{equation}
			\sqrt[|S|]{\prod_{i \in S} i} <= \frac{1}{|S|} \sum_{i \in S} i
			\label{eq:MeanInequality}
		\end{equation}

		\note
		{
			The geometric mean can be changed while preserving the arithmetic mean by redistributing the energy
			in the spectrum.

			The arithmetic mean can be changed by adjusting the relative gains of two bands in a
			particular fashion. Take two bands and apply gain to one and the reciprocal of the gain to the
			other.
		}
