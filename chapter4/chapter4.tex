...
\label{sec:FeatureControl-Parameterisation}
	...
	\label{sec:FetureControl-Parameterisation-Flatness}
		...
		\note
		{
			...
			Two sets comprising the lowest and highest amplitude partials.

			\[ A_{L} = \sum_{n \in L} a_{n}^{2} \]
			\[ A_{U} = \sum_{n \in U} a_{n}^{2} \]
			\begin{equation}
				\textrm{SF} = \frac{N\sqrt[N]{\prod_{n = 1}^{N} a_{n}^{2}}}
				                   {\frac{A_{L}}{m} + mA_{U}}
			\end{equation}
			
			If $\frac{A_{L}}{A_{U}} < m < 1$ the spectral flatness will increase, reaching a maximum
			when $m = \frac{A_{L} + A_{U}}{2A_{U}}$. When $0 < m < \frac{A_{L}}{A_{U}}$ or $m > 1$ the spectral
			flatness will decrease.
		}
