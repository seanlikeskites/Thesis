...
\label{sec:FeatureControl-Parameterisation}
	...
	\label{sec:FeatureControl-Parameterisation-SpectralCentroid}
		The spectral centroid, $f_{\mu}$, of an $N$ sample signal, $x$, is calculated from the $N$ point DFT of the
		signal, $X$, using Equation \ref{eq:SpectralCentroid}.

		\begin{equation}
			f_{\mu} = \frac{f_{s}\sum_{n = 0}^{N-1} |X[n]|(N - |2n - N|)}{2N\sum_{n = 0}^{N-1} |X[n]|}
		...
		spectral centroid to change. A crude method of causing $f_{\mu}$ to move towards a particular frequency is
		the increase the energy at that frequency. While this works conceptually it is destructive to the original
		structure of the spectrum. As $f_{\mu}$ moves towards the desired frequency the spectrum is dominated by a
