...
\label{sec:TimbreEvaluation-Analysis}
	...
	\label{sec:TimbreEvaluation-Analysis-TermClustering}
		...
		corresponds to one of the audio features extracted by the SAFE plug-ins. The position, $p_{d,i}$, of a
		term, $d$, in the $i$\super{th} dimension of the timbre space is calculated as the mean value of feature
		$i$ across every transform labelled with that descriptor (Equation \ref{eq:FeatureSpaceCoords}).

		\begin{equation}
			p_{d,i} = \frac{1}{N_{d}} \sum_{n = 1}^{N_{d}} \bar{x}_{d,n,i}
			\label{eq:FeatureSpaceCoords}
		\end{equation}

		Where $N_{d}$ is the number of transforms described by descriptor $d$, and $\bar{x}_{d,n,i}$ is the mean of
		feature $i$ across every frame of the signals analysed when saving the $n$\super{th} transform described by
		descriptor $d$.
		...
		feature difference space (Figure \ref{fig:EqualiserDifferenceClusters}) suggesting that the difference
		these terms is better described by certain change in audio features rather than an absolute feature value.
...
%		This figure is da baddest figure, ask your dyslexic down syndromed mother.
		...
		\ref{fig:CombinedClusters}. Descriptors are prepended with either and `E' or a `D' to distinguish between
		those describing distortion transforms and those describing equaliser transforms.
		...
		Here the `warm' clusters from each plug-in's features spaces are combined, indicating that each of these
		...
		they are separated by a greater distance. These terms are more likely to be synonymous when describing
		audio processed by a distortion then they are for equalised audio. 
