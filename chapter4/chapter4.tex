...
\label{sec:FeatureControl-Parameterisation}
	...
	\label{sec:FetureControl-Parameterisation-Flatness}
		...
		\ref{eq:PartialSlope} can be altered to produce values in this unit, resulting in Equation
		\ref{eq:PartialOctaveSlope}.

		\[ A = \sum_{n = 1}^{N} 20\log (a_{n}) \quad B = \sum_{n = 1}^{N} 20\log_{2} (\nu_{n})\log (a_{n}) \]
		\[ F = \sum_{n = 1}^{N} \log_{2} (\nu_{n}) \quad G = \sum_{n = 1}^{N} \log_{2} (\nu_{n})^{2} \]
		\begin{equation}
			S = \frac{NB - FA}
		                 {NG - F^{2}}
			\label{eq:PartialOctaveSlope}
		\end{equation}
