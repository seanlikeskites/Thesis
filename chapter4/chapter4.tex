...
\label{sec:TimbreEvaluation-Analysis}
	...
	\label{sec:TimbreEvaluation-Analysis-TermClustering}
		...
		corresponds to one of the audio features extracted by the SAFE plug-ins. The position, $p_{d,i}$ of a term,
		$d$, in the $i$\super{th} dimension of the timbre space is calculated as the mean value of feature $i$
		across every transform labelled with that descriptor (Equation \ref{eq:FeatureSpaceCoords}).

		\begin{equation}
			p_{d,i} = \sum_{n = 1}^{N_{d}} \sum_{f = 1}^{F_{d,n}} \frac{x_{d,n,f,i}}{N_{d}F_{d,n}}
			\label{eq:FeatureSpaceCoords}
		\end{equation}

		Where $N_{d}$ is the number of transforms described by descriptor $d$, $F_{d,n}$ is the number of frames of
		audio analysed when saving the $n$\super{th} transform described by descriptor $d$, and $x_{d,n,f,i}$ is
		the value of feature $i$ in the $f$\super{th} frame of the audio associated with that transform.
