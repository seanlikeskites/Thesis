		...
		\begin{itemize}
			...
			\begin{itemize}
				\item Spectral Centroid ($\mu_{\mathrm{s}}$), Spectral Spread ($\sigma_{\mathrm{s}}^{2}$),
				      ...
				      Irregularity ($\mathrm{JI}$), Krimphoff Irregularity ($\mathrm{KI}$), $f_{0}$,
				      Spectral Smoothness ($\mathrm{SSm}$), Spectral Roll Off ($\mathrm{SRO}$), Spectral
				      Flatness ($\mathrm{SF}$), Tonality ($\tau$), Spectral Crest ($\mathrm{SC}$)
				      and Spectral Slope ($\mathrm{SSl}$).
			\end{itemize}
