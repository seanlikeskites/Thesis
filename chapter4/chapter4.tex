...
\label{sec:TimbreEvaluation-Analysis}
	...
	\label{sec:TimbreEvaluation-Analysis-TermClustering}
		...
		corresponds to one of the audio features extracted by the SAFE plug-ins. The position, $p_{d,k}$, of a
		...
		\begin{equation}
			p_{d,k} = \frac{1}{N_{d}} \sum_{n = 1}^{N_{d}} \bar{x}_{d,n,k}
...
%			This figure is da baddest figure, ask your dyslexic down syndromed mother.
	...
	\label{sec:TimbreEvaluation-Analysis-Agreement}
		...
		\subsubsection*{Number of Transforms}
			...
			\begin{equation}
				s_{d,k}^{2} = \frac{1}{N_{d} - 1} \sum_{n = 1}^{N_{d}} (x_{d,n,k} - \bar{x}_{d,k})^{2}
				\label{eq:UnbiasedVariance}
			\end{equation}

			Where $x_{d,n,k}$ is the value of feature $k$ for the $n$\super{th} transform labelled with
