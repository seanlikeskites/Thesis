...
%			and reversed in time none of the amplitude information in the signal is lost. 
	...
	\label{sec:ExcitationEvaluation-Comparison-TemporalCharacteristics}
		...
		\subsubsection*{Integrator}
			...
			transient signals will be altered. Fast rise times will be lengthened by the attenuation of their
			high frequency components as seen in Figure \ref{fig:IntegratorTemporalEffects}. This prevents
			integrators being used for harmonic excitation in applications where it is essential that
		...
		\subsubsection*{Spectral Replication}
			...
			signal's amplitude envelope by one, leaving it unaffected. Figure
			\ref{fig:SpectralReplicationTemporalEffects} illustrates this, showing the effects on a signal's
			amplitude envelope by shifting it's frequency content upwards by 1.5Hz
	...
	\label{sec:ExcitationEvaluation-Comparison-UseCases}
		...
		\subsubsection*{Multiplier}
			...
			aliasing distortion needs to be eliminated, a harmonic mixer (described in Section
			\ref{sec:ExcitationEvaluation-Comparison-SpectralCharacteristics}) can be used to approximate the
			behaviour of other static nonlinearities.
		...
		\subsubsection*{Spectral Replication}
			...
			should be shifted by. A discussion of different $f_{0}$ tracking methods is given in Section
			\ref{sec:FeatureControl-Systems-Fundamental}. While this introduces considerable complexity the
			bandwidth of the output is bounded at both ends and the system is homogeneous. The spectral
