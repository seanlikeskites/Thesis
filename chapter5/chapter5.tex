...
\label{sec:ExcitationEvaluation-Comparison}
	...
	\label{sec:ExcitationEvaluation-Comparison-Homogeneity}
		...
		\subsubsection*{SSBA}
			...
			by $\abs{x_{a}[n]}^{1-h}$ is equivalent to applying $h$\super{th} order IAP as shown by
		...
		\subsubsection*{Spectral Stretching}
			...
			for each new frame of the \acrshort{stft} are calculated iteratively from previous frames this
			should not occur. When processing the first frame the output signal should have the same phase
			information as the input signal avoiding any unwanted signal inversions.
...
%			and reversed in time none of the amplitude information in the signal is lost. 
	...
	\label{sec:ExcitationEvaluation-Comparison-SpectralCharacteristics}
		...
		\subsubsection*{SSBA}
			...
			Figure~\ref{fig:SSBA3Spectra}, which shows the results of applying $3$\super{rd} order SSBA to the
