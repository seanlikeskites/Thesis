...
\label{sec:ExcitationEvaluation-Comparison}
	...
	\label{sec:ExcitationEvaluation-Comparison-Homogeneity}
		...
		\subsubsection*{IAP}
			...
			being generated $h$. The phase of the amplified signal, $ax_{a}[n]$, scaled by $h$ is $h\pi$ radians
			less than that of the original signal, $x_{a}[n]$, scaled by the same factor (Equation
			...
			To maintain the correct phase information the phase value of a signal which has had negative gain
			applied should be an odd multiple of $\pi$ away from that of the original signal. Equation
			\ref{eq:IAPOrderHomogeneity} shows that this is not the case when $h$ is even. If the input signal
			to a harmonic excitation system using IAP is inverted the odd order harmonics generated will also be
			inverted while the even order ones will not. This could have dramatic effects if the generated
			harmonics are being summed with other signals with the same frequency components.
