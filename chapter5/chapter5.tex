...
\label{sec:ExcitationEvaluation-Comparison}
	...
	\label{sec:ExcitationEvaluation-Comparison-Summary}
		\begin{table}[h!]
			\centering
			\begin{tabular}{|c|c|C{3cm}|C{3cm}|c|c|c|}
				\hline
				...
				Order (odd /  even) and roll off, can be bandlimited & & \tabularnewline
				\hline
				Rectifier & $\mathcal{O}(n)$ & Positive Homogeneous & 
				Even order only (+ $f_{0}$ for half wave) & & \tabularnewline
				\hline
				Integrator & $\mathcal{O}(n)$ & Positive Homogeneous & 
				All orders, shallow roll off, acts as low pass filter & & \tabularnewline
				\hline
				Multiplier & $\mathcal{O}(n)$ & Non-Homogeneous & 
				Integer exponents control maximum order of distortion & & \tabularnewline
				\hline
				...
				Individuals, dense spectra for more complex signals & & \tabularnewline
				\hline
				Spectral Replicator & $\mathcal{O}(n)$ & LTV & 
				Same bandwidth as input just shifted up, retains $f_{0}$ & & \tabularnewline
				\hline
				Spectral Folding & $\mathcal{O}(n)$ & LTV & 
				Little control, fills entire output spectrum & & \tabularnewline
				\hline
				Spectral Stretch & $\mathcal{O}(n\log{n})$ & Homogeneous &
				Scaling of input bandwidth, shifts $f_{0}$ & & \tabularnewline
				\hline
				STTR & $\mathcal{O}(n)$ & LTV & 
				Depends on $f_{0}$ of input, not bounds on distortion components & & \tabularnewline
				\hline
			\end{tabular}
