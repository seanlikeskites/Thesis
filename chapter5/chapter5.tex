...
%		\item Naturalness.
	...
	\label{sec:ExcitationEvaluation-Evaluation-Homogeneity}
		...
		\note
		{
			In section \ref{sec:ExcitationEvaluation-Comparison} the homogeneity of various nonlinear systems
			is discussed. For those which are non-homogeneous methods by which they can be made homogeneous are
			suggested.
		}
...
\label{sec:ExcitationEvaluation-Comparison}
	...
	\label{sec:ExcitationEvaluation-Methods-Statics}
		...
		at or below a given clipping threshold value. Peak clippers are typically piecewise functions comprising
		three sections:
...
%		\subsubsection*{Naturalness}
	...
	\label{sec:ExcitationEvaluation-Methods-SSBA}
		...
		can be build using IIR filters if some properties of an ideal Hilbert transform are disregarded.
...
%		\subsubsection*{Naturalness}
	...
	\label{sec:ExcitationEvaluation-Methods-SpectralReplication}
		...
		Where $f$ is the amount by which the signal's spectrum should be shifted.
...
%		\subsubsection*{Naturalness}
	...
	\label{sec:ExcitationEvaluation-Methods-SpectralStretching}
		...
		\subsubsection*{Complexity}
			...
			\citep{portnoff1976implementation}. When shifting the frequencies upwards, the amount of work can
			be further reduced by resampling the signal before calculating the STFT \citep{laroche1999new}.
			This way there are fewer frames for which the DFT need be calculated.
