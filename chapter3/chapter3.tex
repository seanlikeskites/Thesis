...
\label{sec:Excitation-Methods}
	...
	\label{sec:Excitation-IAP}
		\begin{equation}
			y(n) = |x_{a}(n)| \cos \left( h\arg(x_{a}(n)) \right)
			\label{eq:IAP}
		\end{equation}
	...
	\label{sec:Excitation-SpectralReplication}
		\begin{equation}
			y = \Re \left( x_{a} e^{2\pi fi} \right)
			\label{eq:SectralReplication}
		\end{equation}

	\subsection{Spectral Mirroring}
	\label{sec:Excitation-SpectralMirroring}
		\begin{equation}
			y(n) = \begin{cases}
				-x(n) & \text{if $n$ is odd} \\
				x(n) & \text{otherwise}
			\end{cases}
			\label{eq:SpectralMirror}
		\end{equation}
