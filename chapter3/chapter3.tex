...
\label{sec:Excitation-AnalysisOfNonlinearSystems}
	...
	\label{sec:Excitation-Analysis-Metrics}
		...
		into account by splitting the spectrum of each frame into non-overlapping bands which are one $\mathrm{ERB}$
		in width. The final metric is calculated from the difference in power between these bands in the input and
		output signal according to Equation \ref{eq:DistortionScore}.
		...
		\begin{enumerate}
			...
			\item For each frame and band the maximum value of this cross-correlation, $M_{i,j}$, is found for
				...
				\begin{gather}
					...
					M_{j,i} = \max_{\tau = -\mathrm{T}}^{\mathrm{T}} r_{j,i,\tau}
