...
\label{sec:Excitation-AnalysisOfNonlinearSystems}
	...
	\label{sec:Excitation-Analysis-Modelling}
		...
		\subsubsection{The Synchronised Swept Sine Method}
			...
			the model shown in Figure \ref{fig:HammersteinModel}.
			...
			\begin{figure}[h!]
				...
				\begin{tikzpicture}
					...
					\node (Filter1) [draw] at (3, 4) {$A_{1}[n]$};
					\node (Filter2) [draw] at (3, 3) {$A_{2}[n]$};
					\node (Filter3) [draw] at (3, 2) {$A_{3}[n]$};
					\node (FilterN) [draw] at (3, 0.5) {$A_{p}[n]$};
			...
			The model is, in effect, an extension of a Taylor Series of order $P$. The input is raised to each
			individual power, 1 through $P$, and each of these signals is filtered by an individual kernel,
			$A_{p}[n]$. The resultant signals are then summed to produce the output.

			This method is easier to implement than a wave digital filter as no prior knowledge of the system is
...
\label{sec:Excitation-Uses}
	...
	\label{sec:Excitation-Uses-Reinforcement}
		...
		The pitch of harmonically structured sounds is the same as that of its $f_{0}$. No energy need be present at
...
\label{sec:Excitation-Methods}
	...
	\label{sec:Excitation-Methods-IAP}
		...
		Equation \ref{eq:IAP} shows the $h$\super{th} order instantaneous amplitude and phase modulation of a
		signal.
