...
\label{sec:Excitation-AnalysisOfNonlinearSystems}
	...
	\label{sec:Excitation-Analysis-Components}
		...
		\subsubsection*{Aliasing}
			...
			within the bandwidth allowed by the sampling frequency. The alias frequency, $f_{\textrm{alias}}$,
			...
			\begin{equation}
				f_{\textrm{alias}} = \abs{f - f_{s}\floor{\frac{f}{f_{s}} + \frac{1}{2}}}
				\label{eq:aliasing}
			\end{equation}
	...
	\label{sec:Excitation-Analysis-Metrics}
		...
		methods. These have been denoted $\textrm{THD}_{F}$ and $\textrm{THD}_{R}$ by \citet{shmilovitz2005on} and
		are calculated using Equations \ref{eq:thdf} and \ref{eq:thdr} respectively.

		\begin{equation}
			\textrm{THD}_{F} = \frac{\sqrt{\sum_{n = 2}^{\infty} h_{n}^{2}}}{h_{1}}
			\label{eq:thdf}
		\end{equation}

		\begin{equation}
			\textrm{THD}_{R} = \sqrt{\frac{\sum_{n = 2}^{\infty} h_{n}^{2}}
		...
		Each of these measures has slightly different properties. $\textrm{THD}_{R}$ measures the proportion of the
		output signal which consists of new harmonic components, producing a value between zero and one.
		$\textrm{THD}_{F}$ measures the relative levels of new harmonic content and the energy at the frequency of
		the original sinusoid producing a positive value. \citet{shmilovitz2005on} suggests that $\textrm{THD}_{F}$
		...
		($\textrm{THD}_{F}$ by \citet{fleischmann2014a} and $\textrm{THD}_{R}$ by \citet{dutilleux2011nonlinear}).
		...
		\citet{tan2003the} also developed a perceptual metric for distortion, Distortion Score ($\textrm{DS}$),
		...
		\begin{equation}
			\textrm{DS} = \frac{1}{N} \sum_{n = 1}^{N} \sum_{m = 1}^{M} 
				\abs{20\log_{10} \left( \frac{\textrm{OP}_{n,m}}{\textrm{IP}_{n,m}} \right) }
			\label{eq:DistortionScore}
		\end{equation}
		
		% carry on here
		\citep{tan2003predicting} suggest improvements to the $\textrm{DS}$ metric leading to the
		$R_{\textrm{nonlin}}$ metric. The first improvement is the inclusion of a filter with a frequency response
		similar to that of the outer and middle ear. The filter used is as described by \citet{glasberg2002a}, it
		attenuates frequencies at either end of the audible spectrum, modelling the decreased perception of
		distortion at these frequencies as reported by \citet{voishvillo2006assessment}. This filter is applied to
		the output and input signal before they are split into frames.

		Both these metrics rely on analysing signals which have been processed, so they may not give results which
		can be applied as generally as those using the GedLee metric.

		\citet{tan2004predicting} report greater correlation between $R_{\textrm{nonlin}}$ and perceived audio
		quality then \citet{lee2003auditory} do for the GedLee metric. They also demonstrate how accurate the
		$R_{\textrm{nonlin}}$ metric is in predicting the perceived distortion level introduced to music and speech
		signals.
