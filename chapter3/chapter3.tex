...
\label{sec:Excitation-Methods}
	...
	\label{sec:Excitation-Statics}
		...
		the underlying function is odd.

		Equation \ref{eq:SymmetricHardClipping} is also know as a hard clipper. This is due to the lack of a
		transition section. Soft clippers apply a nonlinear function to medium magnitude samples in order to smooth
		the transition between linear and clipping sections. Equation \ref{eq:SymmetricSoftClipping} shows a soft
		clipper adapted from one given by \citet{dutilleux2002nonlinear}.
		...
		\begin{figure}[h!]
			...
			\caption{Equations \ref{eq:SymmetricHardClipping} and \ref{eq:SymmetricSoftClipping} with a
				 threshold of 0.7.}
			\label{fig:Clipping}
		\end{figure}
