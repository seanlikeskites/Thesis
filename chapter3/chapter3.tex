...
%		\subsubsection*{Naturalness}
	...
	\label{sec:Excitation-Methods-STTR}
		Short-Time time reversal is an audio effect proposed by \citet{kim2014shorttime}. The output signal is
		constructed from overlapping frames of the input signal which have been reversed in time using Equation
		\ref{eq:STTR}.
		...
		\subsubsection*{Complexity}
			The computational complexity of short-time time reversal depends on the ratio of the step size,
			$R$, and the length of the window function $L$ in samples. As $\frac{R}{L}$ decreases the number of
			overlapping frames needed to calculate an output sample increases, increasing the computational
			load.
		...
		\subsubsection*{Homogeneity}
			Short-Time time reversal is a homogeneous process. As the samples in each frame are only multiplied
		...
		\subsubsection*{Spectral Characteristics}
			\note
			{
				All those fun Fourier transform stuffs from Kim's paper.
			}
