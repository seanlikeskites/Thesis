...
\label{sec:Excitation-Introduction}
	...
	varying systems in order to introduce new frequency components to a signal. \citet{dutilleux2011nonlinear} defines
	excitation as the process of controlling timbre through the emphasis of certain frequencies. While this is possible
	with linear systems such as equalisers, nonlinear and time varying systems provide more flexibility as they can add
...
\label{sec:Excitation-AnalysisOfNonlinearSystems}
	...
	\label{sec:Excitation-Analysis-Metrics}
		\subsubsection*{Traditional Metrics}
			...
			Various standards exists for the measurement of IMD. Typically the systems response to a signal
	...
	\label{sec:Excitation-Analysis-Modelling}
		...
		\subsubsection{The Synchronised Swept Sine Method}
			...
			The model is, in effect, an extension of a Taylor Series of order $I$. The input is raised to each
			individual power, 1 through $I$, and each of these signals is filtered by an individual kernel,
			$A_{i}[n]$. The resultant signals are then summed to produce the output.
