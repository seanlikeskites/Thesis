...
\label{sec:Excitation-AnalysisOfNonlinearSystems}
	...
	\label{sec:Excitation-Analysis-Metrics}
		...
		\note
		{
			Clarify the differences in results taken with both measurements.
		}
	...
	\label{sec:Excitation-Analysis-Modelling}
		...
		\subsubsection{The Synchronised Swept Sine Method}
			...
			\note
			{
				Better describe the testing procedure.
			}
...
\label{sec:Excitation-Timbre}
	...
	describe other timbres. The majority of timbral research does not focus on nonlinear distortion and as such does
	not highlight these semantic terms. There have been some publications specifically discussing the timbral effects
	of nonlinear distortion. Their findings are discussed in this section.
...
\label{sec:Excitation-Uses}
	...
	\note
	{
		Perhaps not bullet points. That implies equal emphasis. Also the following sections should follow from the
	...
	\label{sec:Excitation-Uses-Reinforcement}
		...
		\note
		{
			Discuss the actual implementations.
		}
...
%		\item Naturalness.
	...
	\label{sec:Excitation-Evaluation-Homogeneity}
		In order to be intuitive an audio effect it should produce a similar perceptual effect
		across a wide range of input signals. Often this is not the case with traditional audio signal processing
		methods. Take the EQ for example. We can set up an EQ to boost some frequencies in a desired range. The
		spectra of signals with energy in this frequency range will be altered while those of signals with no
		energy in that band will be unaffected.
...
%			constructed.
		...
		\subsubsection*{Aliasing}
			...
			\note
			{
				Why do the higher order harmonics have lower amplitudes.
			}
...
\label{sec:Excitation-Methods}
	...
	\label{sec:Excitation-Methods-Statics}
		...
		applied individually to each sample of a signal. They can be described by a characteristic curve which
...
%		\subsubsection*{Naturalness}
	...
	\note
	{
		Short time time reversal as done by \citet{kim2014shorttime}.
	}
