...
\label{sec:Excitation-Introduction}
	...
	researchers have also worked on creating novel digital distortion techniques \citep{fernandez2001distortion,
	...
	\label{sec:Excitation-Analysis-Metrics}
		...
		\subsubsection{The Synchronised Swept Sine Method}
			...
			\begin{figure}[h!]
				...
				\begin{tikzpicture}
					\node (Sig2) [draw] {$x^{2}(t)$};
					\node (Sig3) [draw, below of=Sig2] {$x^{3}(t)$};
					\node (SigN) [draw, below of=Sig3, yshift=-0.5cm] {$x^{N}(t)$};

					\node (Filter2) [draw, right of=Sig2, xshift=2cm] {$A_{2}(f)$};
					\node (Filter1) [draw, above of=Filter2] {$A_{1}(f)$};
					\node (Filter3) [draw, below of=Filter2] {$A_{3}(f)$};
					\node (FilterN) [draw, right of=SigN, xshift=2cm] {$A_{N}(f)$};

					\draw [arrow] (Sig2) -- (Filter2);
					\draw [arrow] (Sig3) -- (Filter3);
					\draw [arrow] (SigN) -- (FilterN);

					\draw [dots] (Sig3) -- (SigN);
					\draw [dots] (Filter3) -- (FilterN);

					\node (Plus) [operator, right of=Filter3, xshift=2cm] {+};
					\node (In) [left of=Sig3, xshift=-2cm] {$x(t)$};
					\node (Out) [right of=Plus] {$y(t)$};

					\node (Out1) [right of=Filter1, xshift=1.5cm] {};
					\node (Out2) [right of=Filter2, xshift=1.5cm] {};
					\node (Out3) [right of=Filter3, xshift=1.5cm] {};
					\node (OutN) [right of=FilterN, xshift=1.5cm] {};
					...
					\draw [arrow] (Out1) -- (Plus);
					\draw [arrow] (Out2) -- (Plus);
					\draw [arrow] (Out3) -- (Plus);
					\draw [arrow] (OutN) -- (Plus);
				\end{tikzpicture}
