...
\label{sec:Excitation-Timbre}
	...
	descriptors: `sharpness' and `brightness'. In order to discover other timbral properties of nonlinear distortion,
	they performed listening tests in which distorted guitar stimuli were assessed. The stimuli were each processed
	with different nonlinear systems and then further processed so that they had matching Zwicker Sharpnesses (an
	objective measure of sharpness \citep{fastl2007psychoacoustics} discussed in
...
\label{sec:Excitation-Methods}
	...
	\label{sec:Excitation-Methods-SpectralReplication}
		...
		higher frequencies, as shown in Figure~\ref{fig:SpectralReplication}. This shift of energy towards higher
		frequencies could be adapted for use in harmonic excitation systems.  Shifting the spectrum $f$Hz higher in
		frequency is easily implemented through the use of single sideband modulation with a complex sinusoid, as
		shown in Equation~\ref{eq:SpectralReplication}.
	...
	\label{sec:Excitation-Methods-SpectralFolding}
		...
		signal. The effects of spectral folding with $k = 2$ and $k = 3$ are shown in
		Figure~\ref{fig:SpectralFolding}.
		...
		\begin{figure}[h!]
			...
			\caption{The spectral effects of spectral folding.}
			\label{fig:SpectralFolding}
		\end{figure}

	\subsection{Spectral Stretching}
	\label{sec:Excitation-Methods-SpectralStretching}
		\citet{nagel2009a} proposed spectral stretching as a bandwidth extension method. The principle is to scale
		the spectrum of a signal along the frequency axis, as shown in Figure~\ref{fig:SpectralStretching}. A
		typical implementation of this effect utilises a phase vocoder: an algorithm which allows for time
		...
		shorten it. This signal is then resampled back to the length of the original signal, scaling the frequency
		content in the process. Using a hop size of $\delta_{a}$ when calculating the \acrshort{stft} and a hop
		size of $\delta_{s}$ when resynthesising the signal, the spectrum is scaled by a factor, $s$, given by
		Equation~\ref{eq:PhaseVocoderFactor}.

		\begin{figure}[h!]
			\centering
			\includegraphics{chapter3/Images/SpectralStretchingSpectrum.pdf}
			\caption{The spectral effects of spectral stretching.}
