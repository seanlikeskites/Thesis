...
%		\subsubsection*{Naturalness}
	...
	\label{sec:Excitation-Rectification}
		...
		\subsubsection*{Spectral Characteristics}
			...
			curve. As such it only introduces even order distortion components. The output from a half wave
			rectifier has the same spectrum as that of a full wave rectifier but with the content of the
			original signal included. This can easily be shown by considering how a half wave rectified signal
			can be constructed from the original signal and a full wave rectified signal. Where $x(n)$,
			$x_{f}(n)$ and $x_{h}(n)$ represent the original signal, full wave rectified and half waves
			rectified signals respectively, their relationship can be seen in Equation
			...
			\note{may as well put in the Fourier coefficients of a rectified sine wave}
