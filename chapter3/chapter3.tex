...
\label{sec:Excitation-Methods}
	...
	\label{sec:Excitation-Statics}
		...
		\subsubsection*{Aliasing}
			...
			particularly susceptible to aliasing. The steeper roll off produced by soft clippers reduces the
			number of aliased frequencies produced. Other than using a `softer' characteristic curve, there are
			some other techniques to reduce aliasing in static nonlinearities.
			...
			\cite{fernandez-cid2001distortion} construct characteristic curves from Chebyshev polynomials. In
			this fashion the maximum order of polynomial used can be set thus setting the maximum order of the
			distortion components. A conceptually similar approach to this is to create an approximation of the
			characteristic curve by linear regression as shown in Figure \ref{fig:ClippingApproximation}. The
			...
			of having to evaluate a more complex polynomial to calculate the output value for each sample.
		...
		\subsubsection*{Flexibility}
			...
			It is likely that there be inharmonic frequency components among those introduced due to high
			levels of intermodulation distortion and possible aliasing. This excludes these techniques from
			being used in situations were only harmonic distortion is desired.
...
%		\subsubsection*{Naturalness}
	...
	\label{sec:Excitation-Rectification}
		...
		\subsubsection*{Spectral Characteristics}
			...
			\[ a_{0} = \frac{1}{2\pi} \int_{-\pi}^{\pi} |sin(x)| dx \]
			\[ a_{0} = \frac{2}{\pi} \]

			\[ a_{n} = \frac{1}{\pi} \int_{-\pi}^{\pi} |sin(x)|cos(nx) dx \]
			\begin{equation}
				a_{n} = \begin{cases}
					\frac{4}{\pi(1 - n^{2})} & \text{when $n$ is even} \\
					0 & \text{when $n$ is odd}
				\end{cases}
				\label{eq:RectificationFourier}
			\end{equation}

			This represents all even order harmonics rolling off at approximately 12dB per octave.
		...
		\subsubsection*{Aliasing}
			As with other static nonlinearities rectifiers are susceptible to aliasing. Both upsampling and
			approximation with low order polynomials are valid techniques to reduce the amount of aliasing.
		...
		\subsubsection*{Flexibility}
			...
			controlled through filtering to provide more flexibility. Rectifiers also exhibit the same high
			levels of intermodulation distortion as other static nonlinearities when processing musical
			signals. Where only harmonic components are desired other techniques should be used.
...
%		\subsubsection*{Naturalness}
	...
	\label{sec:Excitation-Integrator}
		...
		Where $c$ is the integration constant, effectively setting the output gain of the system. The output signal
		...
		\subsubsection*{Spectral Characteristics}
			...
			\[ A_{n} = \frac{cf_{s}}{2\pi} \left( \int_{0}^{\pi} (1 - cos(x))e^{-inx} dx +
							       \int_{\pi}^{2\pi} (3 + cos(x))e^{-inx} dx \right) \]

			\[ A_{0} = 2cf_{s} \]
			\[ A_{1} = \frac{2icf_{s}}{\pi} \]
			\begin{equation}
				A_{n} = \frac{icf_{s}}{2\pi} \left( \frac{4n^{2} + 2e^{-i\pi n} - 2}{n^{3} - n} \right)
			...
			While integrators are homogeneous they are frequency dependant. They act as low pass filters. Using
			the same integration constant a high frequency inputs signals will produce a lower amplitude output
		...
		\subsubsection*{Aliasing}
			Due to the shallow roll of of the distortion components amplitudes integrators are very prone to
			aliasing. The levels of aliasing can be reduced using the upsampling / downsampling technique
			described in Section \ref{sec:Excitation-Statics}.
